...
- [x] 阶段1：骨架搭建
- [x] 阶段2：专家池 CRUD
- [x] 阶段3：创造模式（LLM 对话式生成档案）
- [x] 阶段4：记忆库道具（OB 文件直读 + 记忆压缩层）
- [x] 阶段5：协作模式（团长 LLM 智能调度 + DAG 编排）
- [x] 阶段6：能力集成（LLM 适配层 + 10 种子专家 + 模型路由）
- [x] 阶段7：Web UI 监控面板（暗色主题 Dashboard + API + 聊天工具）
- [x] 阶段8：测试（22 个用例，覆盖核心模块）
- [ ] 阶段9：打包发布（待 dsh 运行时验证后发布）

## ⚙️ 环境变量

| 变量 | 说明 | 默认 |
|---|---|---|
| `DSH_HOME` | dsh 主目录 | `~/.dsh` |
| `DSH_DEFAULT_MODEL` | 默认模型 | dsh 运行时配置 |
| `DSH_OB_VAULT_PATH` | OB 知识库路径 | 不启用 |
| `DSH_MEMORY_BUS` | 记忆总线开关 | `true` |
